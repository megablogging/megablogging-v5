...
$total = $db->num_rows("select id from article where user='$admin_id'");
if ($total == 0){
	echo "No Data Found!";
	exit();
}
...
?>
			<table class="table table-hover table-bordered">
				  ...
				  <?PHP
					...
					$data_art = $db->select('article', "id, title, category, date, time, hits", "user=$admin_id", "article.date DESC, article.time DESC", "$start, $max_per_table");
				  ...
				  ?>
                   <tr>
                      ...
                      <td>
					  <div class="btn-group">
						  <a target='_blank' href="<?PHP echo "$c_go_url/post/$data[id]"; ?>" class="btn btn-xs btn-primary" title='View Post'> <i class="fa fa-search"></i> </a> 
						  <a href="<?PHP echo "edit_post.mgb?id=$data[id]"; ?>" class="btn btn-xs btn-success" title='Edit Post'> <i class="fa fa-edit"></i> </a>
						  <a href="post.mgb?act=delete&id=<?PHP echo $data['id']; ?>" class="btn btn-xs btn-danger"> <i class="fs fs-remove" title='Delete This Post'></i> </a> 
					  </div>
					  </td>
                    </tr>
				   <?PHP
				...
				<?PHP
					...
					$calc = $max_per_table * $page;
					$start = $calc - $max_per_table;
					$data_art = $db->select('article', "id, title, category, date, time, hits", "title like '%$q%' or content like '%$q%' or category like '%$q%' and user='$admin_id'", "article.date DESC, article.time DESC", "$start, $max_per_table");
				   ...
				   ?>
							...
							<div class="bp-docs-example" style='margin-top:-30px;cursor:pointer;'>
									...
									<ul class='pagination'>
										<?PHP
										if(isset($page)){
										$total = $db->num_rows("select id from article where title like '%$q%' or content like '%$q%' or category like '%$q%' and user='$admin_id'");
										$totalPages = ceil($total / $max_per_table);
										if ($totalPages == 0){
										$totalPages = 1;
										}
